...
- Переживаемость перезагрузки роутера не проверена на практике.
- `/etc/init.d/zeroblock restart` из консоли (не через LuCI) может уронить прокси без автовосстановления — чинится вручную: `/etc/init.d/homevpn-proxy restart`.
