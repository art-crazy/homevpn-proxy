...
Есть также опциональная кнопка «Проверить и починить» — по SSH делает на роутере ту же реальную проверку, что и cron, и перезапускает сервис при отказе. Требует один раз сохранить в приложении IP/логин/пароль роутера — пароль хранится в `%LOCALAPPDATA%\HomeVpnProxyTray\router.json` зашифрованным через Windows DPAPI (привязан к вашей учётке на этом ПК, не хранится в открытом виде и никуда не отправляется). Это добровольная функция — без сохранённых данных кнопка просто неактивна, остальной функционал приложения от неё не зависит.
